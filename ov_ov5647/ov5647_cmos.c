...
SNS_COMBO_DEV_ATTR_S *g_pastOv5647ComboDevArray[VI_MAX_PIPE_NUM] = {CVI_NULL};
...
#define OV5647_SENSOR_GET_COMBO(dev, pstCtx)   (pstCtx = g_pastOv5647ComboDevArray[dev])
#define OV5647_SENSOR_SET_COMBO(dev, pstCtx)   (g_pastOv5647ComboDevArray[dev] = pstCtx)
...
ISP_SNS_COMMADDR_U g_aunOv5647_AddrInfo[VI_MAX_PIPE_NUM] = {
	[0] = { .s8I2cAddr = 0},
...
static CVI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *pstSnsSyncInfo)
{
	...
	if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
		...
		for (i = 0; i < pstCfg0->snsCfg.u32RegNum; i++) {
			...
			pstI2c_data[i].u8DevAddr = g_aunOv5647_AddrInfo[ViPipe].s8I2cAddr;
...
static CVI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
	...
	if (pstSensorImageMode->f32Fps <= 30) {
		if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
			if (OV5647_RES_IS_1920P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	OV5647_SENSOR_GET_CTX(ViPipe, pstSnsState);
	OV5647_SENSOR_GET_COMBO(ViPipe, pstRxAttrSrc);
	...
	pstRxAttr->img_size.start_x = g_astOv5647_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32X;
	pstRxAttr->img_size.start_y = g_astOv5647_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32Y;
	pstRxAttr->img_size.active_w = g_astOv5647_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Width;
	pstRxAttr->img_size.active_h = g_astOv5647_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Height;
	...
	pstRxAttr->img_size.max_width = g_astOv5647_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Width;
	pstRxAttr->img_size.max_height = g_astOv5647_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Height;
...
static CVI_S32 sensor_patch_rx_attr(VI_PIPE ViPipe, RX_INIT_ATTR_S *pstRxInitAttr)
{
	...
	if (!g_pastOv5647ComboDevArray[ViPipe]) {
		pstRxAttr = malloc(sizeof(SNS_COMBO_DEV_ATTR_S));
	} else {
		OV5647_SENSOR_GET_COMBO(ViPipe, pstRxAttr);
	}
	memcpy(pstRxAttr, &ov5647_rx_attr, sizeof(SNS_COMBO_DEV_ATTR_S));
	OV5647_SENSOR_SET_COMBO(ViPipe, pstRxAttr);
...
{
	if (g_pastOv5647ComboDevArray[ViPipe]) {
		free(g_pastOv5647ComboDevArray[ViPipe]);
		g_pastOv5647ComboDevArray[ViPipe] = CVI_NULL;
	}
...
static CVI_VOID sensor_patch_i2c_addr(VI_PIPE ViPipe, CVI_S32 s32I2cAddr)
{
	if (OV5647_I2C_ADDR_IS_VALID(s32I2cAddr)) {
		g_aunOv5647_AddrInfo[ViPipe].s8I2cAddr = s32I2cAddr;
	} else {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C addr input error ,please check [0x%x]\n", s32I2cAddr);
		g_aunOv5647_AddrInfo[ViPipe].s8I2cAddr = OV5647_I2C_ADDR_1;
	}
...
static CVI_VOID sensor_ctx_exit(VI_PIPE ViPipe)
{
	...
	g_aeOv5647_MirrorFip[ViPipe] = ISP_SNS_NORMAL;
}
